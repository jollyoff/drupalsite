...
jQuery(document).ready(function($) {
  ...
  var currentIndex = 0;
  var items = $('.views-row .views-col'); // Уточнение селектора
  var totalItems = items.length;

  // Добавление кнопок навигации к родительскому контейнеру .views-row
  $('.block-views-blockgolovne-block-1').append('<button class="prev">Назад</button><button class="next">Вперед</button>');
  ...
  function toggleSlide(direction) {
    ...
    $('.views-row .views-col').css('transform', 'translateX(' + translateValue + ')');
  }
